...
interface Ctx {
  ...
  nextNumId: number; // 有序列表编号实例分配器（从 2 起；numId 1 固定为项目符号）
  numList: { numId: number; start: number }[]; // 分配的 ol 编号实例（含自定义起始号），供 numbering.xml 声明
}
...
/** 文本 → run 序列（软换行 \n 并入空格，与 HTML 路径空白合并语义一致；硬换行 break 节点仍输出 w:br） */
function textToRuns(text: string, fmt: RunFmt): string {
  const parts = text.split('\n');
  let out = '';
  for (let i = 0; i < parts.length; i++) {
    if (i > 0 && parts[i]) out += plainRun(' ', fmt);
...
function blockToXml(
  ...
  switch (node.type) {
    ...
    case 'paragraph': {
      ...
      const prefix = extra?.prefix ? plainRun(extra.prefix, {}) : '';
      const headerFmt = extra?.headerBold ? headerBoldFmt({}) : {};
      // 独立成段图片居中：段落唯一子节点为图片，且非表格单元格/列表项/引用块
      const children = node.children as unknown[];
      const isStandaloneImage =
        !extra?.style && !extra?.numPr && children.length === 1 && (children[0] as { type?: string }).type === 'image';
      const centerJc = isStandaloneImage ? '<w:jc w:val="center"/>' : '';
      const allPPr = `${pPr}${centerJc}`;
      const hasBlockMath = children?.some(
        (c) => (c as { type?: string }).type === 'math' && (c as { display?: boolean }).display
      );
      if (!hasBlockMath) {
        return `<w:p>${allPPr ? `<w:pPr>${allPPr}</w:pPr>` : ''}${prefix}${inlineChildren(children as never[], ctx, headerFmt)}</w:p>`;
      }
      return splitParagraphAtBlockMath(node as { children: unknown[] }, ctx, {
        ...extra,
        style: pStyle,
...
<w:style w:type="paragraph" w:styleId="Heading1"><w:name w:val="heading 1"/><w:basedOn w:val="Normal"/><w:next w:val="Normal"/><w:qFormat/><w:pPr><w:keepNext/><w:spacing w:before="240" w:after="120"/><w:outlineLvl w:val="0"/></w:pPr><w:rPr><w:rFonts w:eastAsia="黑体"/><w:b/><w:sz w:val="32"/><w:szCs w:val="32"/></w:rPr></w:style>
<w:style w:type="paragraph" w:styleId="Heading2"><w:name w:val="heading 2"/><w:basedOn w:val="Normal"/><w:next w:val="Normal"/><w:qFormat/><w:pPr><w:keepNext/><w:spacing w:before="240" w:after="120"/><w:outlineLvl w:val="1"/></w:pPr><w:rPr><w:rFonts w:eastAsia="黑体"/><w:b/><w:sz w:val="28"/><w:szCs w:val="28"/></w:rPr></w:style>
<w:style w:type="paragraph" w:styleId="Heading3"><w:name w:val="heading 3"/><w:basedOn w:val="Normal"/><w:next w:val="Normal"/><w:qFormat/><w:pPr><w:keepNext/><w:spacing w:before="240" w:after="120"/><w:outlineLvl w:val="2"/></w:pPr><w:rPr><w:rFonts w:eastAsia="黑体"/><w:b/><w:sz w:val="26"/><w:szCs w:val="26"/></w:rPr></w:style>
<w:style w:type="paragraph" w:styleId="Heading4"><w:name w:val="heading 4"/><w:basedOn w:val="Normal"/><w:next w:val="Normal"/><w:qFormat/><w:pPr><w:keepNext/><w:spacing w:before="240" w:after="120"/><w:outlineLvl w:val="3"/></w:pPr><w:rPr><w:rFonts w:eastAsia="黑体"/><w:b/><w:sz w:val="24"/><w:szCs w:val="24"/></w:rPr></w:style>
<w:style w:type="paragraph" w:styleId="Heading5"><w:name w:val="heading 5"/><w:basedOn w:val="Normal"/><w:next w:val="Normal"/><w:qFormat/><w:pPr><w:keepNext/><w:spacing w:before="240" w:after="120"/><w:outlineLvl w:val="4"/></w:pPr><w:rPr><w:rFonts w:eastAsia="黑体"/><w:b/><w:sz w:val="22"/><w:szCs w:val="22"/></w:rPr></w:style>
<w:style w:type="paragraph" w:styleId="Heading6"><w:name w:val="heading 6"/><w:basedOn w:val="Normal"/><w:next w:val="Normal"/><w:qFormat/><w:pPr><w:keepNext/><w:spacing w:before="240" w:after="120"/><w:outlineLvl w:val="5"/></w:pPr><w:rPr><w:rFonts w:eastAsia="黑体"/><w:b/><w:sz w:val="22"/><w:szCs w:val="22"/></w:rPr></w:style>
...
export function toDocx(files: Map<string, Uint8Array>, opts: DocxOptions = {}, onWarning?: (msg: string) => void): Uint8Array {
  ...
  const ctx: Ctx = {
    ...
    docPrId: 1,
    nextNumId: 2, // numId 1 固定绑定项目符号 abstractNum
    numList: [],
  };
