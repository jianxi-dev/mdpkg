# folder-open Specification

## Purpose
浏览器端文件夹/多条目拖入直开能力：支持拖入文件夹（含 .md 与同级附件资源）、多选文件同拖、单文件拖入三种输入，递归收集目录树，保留相对路径构造包 Map，按 lenient 语义渲染（图片内联）。通过 `openFiles` 统一入口暴露给 mdpkg-web。
